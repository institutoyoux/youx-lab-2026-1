...
select nome,
      coalesce(extract(month from data_nascimento),'0') as Mes from cliente;
...
select nome,case extract(month from data_nascimento)
            when 1 then 'Janeiro'
...
select nome,
      coalesce(extract(year from data_nascimento),'0') as Ano from cliente;

--4. caractere 5 até o caractere 10 de todos os municípios.
