...
btnCadastrarTarefa.addEventListener("click", function () {
    if (nomeTarefa.value.trim() === "" || dataTarefa.value === "" || dataTerminoTarefa.value === "") {
        alert("Preencha todos os campos!");
        return;
    }

    if (dataTerminoTarefa.value < dataTarefa.value) {
        alert("A data de término não pode ser anterior à data de cadastro!");
        return;
    }

    const tipoTexto = tiposTarefa.options[tiposTarefa.selectedIndex].text;
    const statusTexto = statusTarefa.options[statusTarefa.selectedIndex].text;
    const prazo = calcularPrazo(dataTerminoTarefa.value);
...
function criarLinha(nome, tipo, dataTermino, statusTexto, statusValor, prazo) {
    return `
        <td>${nome}</td>
        <td>${tipo}</td>
        <td>${formatarData(dataTermino)}</td>
        <td><span class="${classeStatus(statusValor)}">${statusTexto}</span></td>
        <td><span class="${prazo.classe}">${prazo.texto}</span></td>
        <td>
            <button type="button" class="btnEditar">Editar</button>
            <button type="button" class="btnExcluir">Deletar</button>
        </td>
    `;
}
...
function calcularPrazo(dataFinal) {
    ...
    const termino = new Date(dataFinal + "T00:00:00");
    const diferenca = termino - hoje;
    const dias = Math.ceil(diferenca / (1000 * 60 * 60 * 24));

    if (dias < 0) {
        return { texto: "ATRASADO", classe: "prazoAtrasado" };
    }

    if (dias === 0) {
        return { texto: "HOJE", classe: "prazoEmDia" };
    }
...
function classeStatus(status) {
    if (status === "finalizado") return "statusFinalizado";
    if (status === "em-andamento") return "statusEmAndamento";
...
listaTarefas.addEventListener("click", function (evento) {
    if (evento.target.classList.contains("btnExcluir")) {
    ...
    if (evento.target.classList.contains("btnEditar")) {
        ...
        const data = colunas[2].textContent.trim();
        const status = colunas[3].querySelector("span");

        nomeTarefa.value = nome;

        const partesData = data.split("/");
        dataTerminoTarefa.value = `${partesData[2]}-${partesData[1]}-${partesData[0]}`;
        ...
        if (status.classList.contains("statusFinalizado")) {
            statusTarefa.value = "finalizado";
        } else if (status.classList.contains("statusEmAndamento")) {
            statusTarefa.value = "em-andamento";
        } else {
            statusTarefa.value = "nao-iniciado";
        }

        linhaEditando = linha;
        btnCadastrarTarefa.textContent = "Salvar Alterações";
    }
});